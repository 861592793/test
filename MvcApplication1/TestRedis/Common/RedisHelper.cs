...
namespace TestRedis.Common
{
    public class RedisHelper
    {
        ...
        public void addLock(string lockKey,string token)
        {
            bool isSucess = false;
            while (!isSucess)
            {
                isSucess = _db.LockTake(lockKey, token, TimeSpan.FromSeconds(20));
            }
        }
        ...
        {
            return string.Format("{0}_{1}",DefaultKey,key);
        }
